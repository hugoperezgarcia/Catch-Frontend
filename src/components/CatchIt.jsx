...
export function CatchIt() {
  ...
  const randomizarRespuestas = () =>{
    ...
    Object.keys(preguntas[numPreguntaActual]).forEach(clave =>{
      ...
      if(clave.includes("respuestaCorrecta")){
        respuestasCorrectas.push(preguntas[numPreguntaActual][clave]);
      }
    })
    ...
    getRespuestaCorrecta(arrayRespuestas);
  }

  function getRespuestaCorrecta(arrayRespuestas){
    for(let i = 0; i < arrayRespuestas.length; i++){
      if(arrayRespuestas[i].includes(respuestasCorrectas[Number(numPreguntaActual)])){
        console.log("trampilla correcta "+(i+1));
 ...
 function resetear(){
    ...
    animarTrampillas();
    setTimeout(function() {
      sessionStorage.setItem("numPreguntaActual", Number(numPreguntaActual) + 1)
      resetearTrampillas();
  ...
  function cambiarColorPregunta() {
    document.getElementById(`numPreg${numPreguntaActual}`).classList.replace("border-red-500", "border-green-300");
    document.getElementById(`numPreg${numPreguntaActual}`).classList.replace("bg-red-200", "bg-green-600");
    document.getElementById(`numPreg${numPreguntaActual}`).classList.add("text-white");
  }
  ...
  function animarTrampillas(){ 
    for(let i = 1; i < 5; i++){
    if(document.getElementById(`cont${i}`) !== document.getElementById(`cont${numTrampillaCorrecta}`)){
      ...
      <header className="flex justify-between h-3/5">
        <div className="mx-5">
          <div className="flex items-center">
            ...
            <button onClick={resetear}  className="w-14 ring-white ring-2 shadow-md shadow-azul-oscuro bg-azul-oscuro flex justify-center rounded-lg font-thin text-white h-9 items-center">
              <LogoSkip />
            </button>
          </div>
        ...
        <div className="h-full border-black border-2 rounded-lg mt-3 me-5 flex-grow flex flex-col items-center justify-around bg-gradient-to-b from-blue-600 to-blue-300">
          <div className="flex mt-3 w-full justify-around">
            <div id="numPreg0" className="rounded-full border-2 border-red-500 w-10 h-10 flex justify-center items-center font-medium bg-red-200">1</div>
            <div id="numPreg1" className="rounded-full border-2 border-red-500 w-10 h-10 flex justify-center items-center font-medium bg-red-200">2</div>
            <div id="numPreg2" className="rounded-full border-2 border-red-500 w-10 h-10 flex justify-center items-center font-medium bg-red-200">3</div>
            <div id="numPreg3" className="rounded-full border-2 border-red-500 w-10 h-10 flex justify-center items-center font-medium bg-red-200">4</div>
            <div id="numPreg4" className="rounded-full border-2 border-red-500 w-10 h-10 flex justify-center items-center font-medium bg-red-200">5</div>
            <div id="numPreg5" className="rounded-full border-2 border-red-500 w-10 h-10 flex justify-center items-center font-medium bg-red-200">6</div>
            <div id="numPreg6" className="rounded-full border-2 border-red-500 w-10 h-10 flex justify-center items-center font-medium bg-red-200">7</div>
            <div id="numPreg7" className="rounded-full border-2 border-red-500 w-10 h-10 flex justify-center items-center font-medium bg-red-200">8</div>
          </div>
          <div id="enunciadoPregunta" className="p-8 font-medium text-4xl w-full text-center animate-fade-down animate-ease-in">
            {preguntas[numPreguntaActual].pregunta}
          </div>
          <div className="flex justify-around w-full mb-2 gap-1 m-1">
            <div id="res1" className="border-2 border-black rounded-md w-60 h-fit min-h-28 flex flex-col items-center justify-around bg-azul-oscuro text-white animate-fade-down animate-delay-[1000ms] animate-ease-in"><div className="font-medium">A</div><div>{respuestas[0]}</div></div>
            <div id="res2" className="border-2 border-black rounded-md w-60 h-fit min-h-28 flex flex-col items-center justify-around bg-azul-oscuro text-white animate-fade-down animate-delay-[2000ms] animate-ease-in"><div className="font-medium">B</div><div>{respuestas[1]}</div></div>
            <div id="res3" className="border-2 border-black rounded-md w-60 h-fit min-h-28 flex flex-col items-center justify-around bg-azul-oscuro text-white animate-fade-down animate-delay-[3000ms] animate-ease-in"><div className="font-medium">C</div><div>{respuestas[2]}</div></div>
            <div id="res4" className="border-2 border-black rounded-md w-60 h-fit min-h-28 flex flex-col items-center justify-around bg-azul-oscuro text-white animate-fade-down animate-delay-[4000ms] animate-ease-in"><div className="font-medium">D</div><div>{respuestas[3]}</div></div>
          </div>
        </div>
      </header>
            ...
            <div className="flex justify-around w-full mb-2 gap-1 m-1">
              <div id="cont1" className="h-52 w-60 ring-4 ring-azul-oscuro rounded-lg bg-zinc-400 flex flex-col justify-between shadow-lg shadow-azul-oscuro">
                <div className="flex justify-between items-start w-full text-4xl font-semibold">
                  <button className="m-1 border-4 border-green-500 px-1 bg-green-300 rounded" onClick={() => handleIncrement(0)}>+</button>
                  <button className="m-1 border-4 border-red-500 px-1 bg-red-300 rounded" onClick={() => handleDecrement(0)}>-</button>
                </div>
              ...
              <div id="cont2" className="h-52 w-60 ring-4 ring-azul-oscuro rounded-lg bg-zinc-400 flex flex-col justify-between shadow-lg shadow-azul-oscuro">
                <div className="flex justify-between items-start w-full text-4xl font-semibold">
                  <button className="m-1 border-4 border-green-500 px-1 bg-green-300 rounded" onClick={() => handleIncrement(1)}>+</button>
                  <button className="m-1 border-4 border-red-500 px-1 bg-red-300 rounded" onClick={() => handleDecrement(1)}>-</button>
                </div>
                <div className="flex justify-center">
                  {puntosApostados[1]}
                </div>
              ...
              <div id="cont3" className="h-52 w-60 ring-4 ring-azul-oscuro rounded-lg bg-zinc-400 flex flex-col justify-between shadow-lg shadow-azul-oscuro">
                <div className="flex justify-between items-start w-full text-4xl font-semibold">
                  <button className="m-1 border-4 border-green-500 px-1 bg-green-300 rounded" onClick={() => handleIncrement(2)}>+</button>
                  <button className="m-1 border-4 border-red-500 px-1 bg-red-300 rounded" onClick={() => handleDecrement(2)}>-</button>
                </div>
                <div className="flex justify-center">
                  {puntosApostados[2]}
                </div>
              ...
              <div id="cont4" className="h-52 w-60 ring-4 ring-azul-oscuro rounded-lg bg-zinc-400 flex flex-col justify-between shadow-lg shadow-azul-oscuro">
                <div className="flex justify-between items-start w-full text-4xl font-semibold">
                  <button className="m-1 border-4 border-green-500 px-1 bg-green-300 rounded" onClick={() => handleIncrement(3)}>+</button>
                  <button className="m-1 border-4 border-red-500 px-1 bg-red-300 rounded" onClick={() => handleDecrement(3)}>-</button>
                </div>
                <div className="flex justify-center">
                  {puntosApostados[3]}
                </div>
